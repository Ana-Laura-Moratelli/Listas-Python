#A)Questão A. O que o seguinte programa (dado na forma de pseucódigo) imprime?
#x = 2
#y = 5
#se y > 8 então
#y = y * 2
...
#B)Questão B. Quantas vezes o trecho de pseudocódigo seguinte imprime 'oi'? (obs: na nossa pseudo-linguagem, o laço inclui os extremos, ou seja, 1 até 4 significa 1, 2, 3, 4.)
#para i = 1 até 9
#se i != 3, então
#para j = 1 até 6
#imprime 'oi'
...
#C)Questão C. Entre 1067 e 3627 (inclusive), quantos números são pares e também divisíveis por 7? Resposta: 183
cont = 0
...
#D)Questão D. Daniela é uma pessoa muito supersticiosa. Para ela, um número é sortudo se ele contém o dígito 2 mas não o dígito 7. Então, na opinião dela, quantos números sortudos existem entre 18644 e 33087, incluindo os extremos? Resposta: 7995
cont = 0
...
#E)Questão E. Na pacata vila campestre de Ponteironuloville, todos os telefones têm 6 dígitos. A companhia telefônica estabelece as seguintes regras sobre os números:
...
cont = 0
for x in telefones:
    if not dois(x) and par(x) and x[0] != x[-1]:
        cont = cont + 1
print('Nessa lista são válidos:',cont,'números')
