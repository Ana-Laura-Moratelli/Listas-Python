...
for x in txtA:
    if x[0] in zumbi and x[-1] not in zumbi:
        cont= cont + 1
print('No texto A exixtem:',cont,'preposições')

cont = 0
for x in txtB:
    if x[0] in zumbi and x[-1] not in zumbi:
        cont = cont + 1
print('No texto B exixtem:',cont,'preposições')

cont = 0
for x in txtA:
    if len(x) == 7 and x[-1] not in zumbi:
        cont = cont + 1
print('No texto A são:',cont,'verbos')

cont = 0
for x in txtA:
    if len(x) == 7 and x[0] not in zumbi and x[-1] not in zumbi:
        cont = cont + 1
print('No texto A são:',cont,'verbos em primeira pessoa')

cont = 0
for x in txtB:
    if len(x) == 7 and x[-1] not in zumbi:
        cont = cont + 1
print('No texto B são:',cont,'verbos')

cont = 0
for x in txtB:
    if len(x) == 7 and x[0] not in zumbi and x[-1] not in zumbi:
        cont = cont + 1
print('No texto B são:',cont,'verbos em primeira pessoa')
...
def google(x):
    ...
    resp = ''
    #vou deixar em maiúsulo para nao misturar com a ordem

    for letra in x:
        pos = ordem.find(letra)
        resp = resp + normal[pos]
    return resp
txtA.sort(key=google)
print(txtA)
...
def valor(n):
    ...
    ordem = 'zmbtshjpnwlrcxkqvdgf'
    for x in n:
        soma = soma + ordem.find(x)*p20
        p20 = p20 * 20
    return soma

def magico(n):
    if valor(n) % 42 == 0 and len(n) == len(set(n)):
        return n
